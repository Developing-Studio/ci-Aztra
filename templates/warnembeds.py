    ...
    async def ko(self, pgr: Pager, member):
        ...
        if not warns:
            return discord.Embed(
                title=f'🚨 | {member} 님의 경고가 하나도 없습니다!',
                color=self.cog.color['warn']
            )

        embed = discord.Embed(
            title=f'🚨 | {member} 님의 경고 목록',
            description='',
            color=self.cog.color['warn']
        )
        ...
        for one in warns:
            ...
            """.format(
                one['reason'],
                one['count'],
                ' '.join(list(timedelta.format_timedelta(now-one['dt']).values())[0]) + ' 전'
...
class Warn_give_ask(aEmbedBase):
    async def ko(self, target, reason, count):
        return discord.Embed(
            ...
            name='대상', value=target.mention
        ).add_field(
            name='사유', value=reason if reason else '(없음)'
        ).add_field(
            name='경고 횟수', value=str(count)
        )