...
class Help(aEmbedBase):
    async def ko(self):
        embed = discord.Embed(title='📃 Aztra 전체 명령어', description='(소괄호)는 필수 입력, [대괄호]는 선택 입력입니다.\n\n', color=self.cog.color['primary'])
        ...
        return embed
    async def en(self):
        embed = discord.Embed(title='📃 Aztra All Commands', description='(Round brackets) are required, [Square brackets] are optional.\n\n', color=self.cog.color['primary'])
        for name, value in help.gethelps():
            embed.add_field(
                name='🔸' + name,
                value=value.format(p=self.cog.prefix),
...
class Info(aEmbedBase):
    async def ko(self):
        uptimenow = re.findall(r'\d+', str(datetime.datetime.now() - self.cog.bot.get_data('start')))
        uptimestr = ''
        if len(uptimenow) == 4:
        ...
        return discord.Embed(
            title='🏷 Aztra 정보',
            description=f"""\
                **Aztra - 미래를 바꿀 디스코드 관리봇.**
                > Aztra 버전: {self.cog.bot.get_data("version_str")}
                > 실행 시간: {uptimestr}
                > Discord.py 버전: {discord.__version__}
                > **{len(self.cog.bot.guilds)}** 서버 | **{len(self.cog.bot.users)}** 유저
...
class Ping(aEmbedBase):
    ...
    async def en(self, mping):
        ...
        embed.add_field(name='Discord Gateway', value=f'{self.cog.bot.get_data("ping")[0]}ms')
...
class Shard(aEmbedBase):
    async def ko(self):
        gshs = self.cog.bot.get_data("guildshards")
        if gshs:
            ...
            return discord.Embed(description=f'**현재 Aztra는 자동 샤딩을 사용하고 있지 않습니다.**', color=self.cog.color['info'])
    ...
    async def en(self):
        ...
        if gshs:
            ...
            return discord.Embed(description=f"**Currently Aztra doesn't use auto sharding.**", color=self.cog.color['info'])
...
class Register_already_registered(aEmbedBase):
    ...
    async def en(self):
        return discord.Embed(title=f'{self.cog.emj.get(self.ctx, "check")} You already registered to Aztra.', color=self.cog.color['info'])
