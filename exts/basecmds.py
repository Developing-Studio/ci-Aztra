...
class BaseCmds(BaseCog):
    ...
    async def _ext_load(self, ctx: commands.Context, *names):
        if not names or '*' in names:
            loads = list(set(self.bot.get_data('allexts')) - set(self.bot.extensions.keys()))
            try:
                if len(loads) == 0:
                    raise commands.ExtensionAlreadyLoaded('모든 확장이 이미 로드되었습니다.')
                for onename in loads:
                    self.bot.load_extension(onename)
            ...
            try:
                for onename in names:
                    if not (onename in self.bot.get_data('allexts')):
                        raise commands.ExtensionNotFound(f'존재하지 않는 확장: {onename}')
                    if onename in self.bot.extensions:
    ...
    async def _ext_unload(self, ctx: commands.Context, *names):
        if not names or '*' in names:
            unloads = list(self.bot.extensions.keys())
            unloads = list(filter(lambda ext: ext not in self.bot.get_data('lockedexts'), unloads))
            try:
                if len(unloads) == 0:
                    raise commands.ExtensionNotLoaded('로드된 확장이 하나도 없습니다!')
                for onename in unloads:
                    self.bot.unload_extension(onename)
            except commands.ExtensionNotLoaded:
            ...
            try:
                if set(names) >= set(self.bot.get_data('lockedexts')):
                    lockedinnames = ", ".join(set(names) & set(self.bot.get_data("lockedexts")))
                    raise errors.LockedExtensionUnloading('잠긴 확장은 언로드할 수 없습니다: ' + lockedinnames)
                for onename in names:
                    if not (onename in self.bot.extensions):
                        raise commands.ExtensionNotLoaded(f'로드되지 않은 확장: {onename}')
                for onename in names:
                    self.bot.unload_extension(onename)
