...
class Mastercmds(BaseCog):
    def __init__(self, bot):
        ...
        for cmd in self.get_commands():
            cmd.add_check(self.check.has_aztra_permissions(master=True))
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                ...
                for one in guild_dbs:
                    guild = self.bot.get_guild(one['id'])
                    if guild:
                ...
                cpembed = discord.Embed(title='📢 공지 전송', description=f'전체 `{len(self.bot.guilds)}`개 서버 중 유효한 서버 `{len(guilds)}`개 서버에 전송합니다.', color=self.color['primary'])
    ...
    async def _log(self, ctx: commands.Context, arg):
        async with ctx.channel.typing():
            name = arg.lower()
            if name in ['aztra', '아즈트라']:
                f = discord.File(fp='./logs/aztra/aztra.log', filename='aztra.log')
    ...
    async def _shutdown(self, ctx: commands.Context, seconds: typing.Optional[float]=60.0):
        ...
        msg = await ctx.send(embed=discord.Embed(
            title='🖥 Aztra 종료',
            description=f'{timeleftstr}Aztra의 모든 명령어 처리 및 백그라운드 작업이 중단되고 데이터베이스 연결, SSH 연결을 닫습니다.\n**계속합니까?**',
                ...
                if now:
                    ...
                    async def time_left():
                        while time.time() - start < seconds and self.will_shutdown:
                            self.bot.set_data('shutdown_left', seconds - (time.time() - start))
                            await asyncio.sleep(0.1)
                    await time_left()
                else:
                    await ctx.send(embed=discord.Embed(title='지금 Aztra가 종료됩니다.', color=self.color['warn']))
                if self.will_shutdown:
                    await self.shutdown()
                else:
                    self.bot.set_data('shutdown_left', None)
    ...
    async def _restart(self, ctx: commands.Context, seconds: typing.Optional[float]=60.0):
        ...
        msg = await ctx.send(embed=discord.Embed(
            title='🖥 Aztra 재시작',
            description=f'{timeleftstr}Aztra가 완전히 종료된 후 다시 시작됩니다.\n**계속합니까?**',
            color=self.color['warn']
        ))
        emjs = ['⭕', '❌']
        for em in emjs:
        ...
        try:
            ...
            if reaction.emoji == '⭕':
                if now:
                    ...
                    await ctx.send(embed=discord.Embed(title='지금 Aztra가 재시작됩니다.', color=self.color['warn']))
    ...
    async def _inspection_on(self, ctx: commands.Context):
        if self.bot.get_data('on_inspection'):
            await ctx.send(embed=discord.Embed(title='🔐 점검 모드 활성됨', description='이미 점검 모드가 활성되어 있습니다.', color=self.color['warn']))
        else:
            self.bot.add_check(self.check.on_inspection)
            self.bot.set_data('on_inspection', True)
            await ctx.send(embed=discord.Embed(title='🔐 점검 모드 활성됨', description='이제 운영진만 Aztra를 사용할 수 있습니다.', color=self.color['warn']))

    @_inspection.command(name='비활성화', aliases=['끄기', '꺼짐', '해제', '비활성'])
    async def _inspection_off(self, ctx: commands.Context):
        if self.bot.get_data('on_inspection'):
            self.bot.remove_check(self.check.on_inspection)
            self.bot.set_data('on_inspection', False)
            await ctx.send(embed=discord.Embed(title='🔓 점검 모드 해제됨', description='이제 누구든지 Aztra를 사용할 수 있습니다.', color=self.color['warn']))
        else:
